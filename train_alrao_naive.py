...
from alrao_tf import AlraoGradientDescentOptimizer, AlraoMomentumOptimizer
...
def main(args):
    ...
    with tf.name_scope("Optimizer"):
        if args.momentum > 0:
            opt = AlraoMomentumOptimizer(min_lr=args.min_lr, max_lr=args.max_lr, momentum=args.momentum)
        else:
            opt = AlraoGradientDescentOptimizer(min_lr=args.min_lr, max_lr=args.max_lr)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        acc_v, summaries = sess.run([metric_acc, merged_summaries_train], feed_dict=feed_dict)
        ...
        feed_dict = {is_training_bn: False}
        acc_v, summaries = sess.run([metric_acc, merged_summaries_valid], feed_dict=feed_dict)
    ...
    feed_dict = {is_training_bn: False}
    acc_test, summaries = sess.run([metric_acc, merged_summaries_valid], feed_dict=feed_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", dest="output_dir", type=str, default="logs/alrao_naive/")
